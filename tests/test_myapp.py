...
import pytest
from myapp.app import calculate_object_equals_type_all
from myapp.app import ObjectType
from myapp.app import get_args
from myapp.app import get_good_objects
...
def test_object_type_d(tmp_path):
    ...
    """
    mydir = ObjectType(tmp_path)
    new_path = mydir.object_equal_type("dir")
    assert new_path == tmp_path
...
def test_main(tmp_path, dirs, files):
    ...
    assert good_objects == main([str(tmp_path)])
